...
if [[ -S /tmp/mpv-socket ]]; then
    ...
    mpvpaper -o "video-aspect-override=16:9 --panscan=1.0 --hwdec=nvdec --vo=gpu  --loop --no-audio --input-ipc-server=/tmp/mpv-socket" eDP-1 "$VIDEO_FILE" &
fi    
...
if ss -xln | grep -q "$SOCKET_PATH"; then
    ...
    pkill mpvpaper
    echo "Starting mpvpaper..."
    mpvpaper -o "video-aspect-override=16:9 --panscan=1.0 --hwdec=nvdec --vo=gpu --loop --no-audio --input-ipc-server=$SOCKET_PATH" eDP-1 "$OUTPUT_FILE" &
    sleep 2
fi
